...
func TestReportShouldAddRow(t *testing.T) {
	r := NewReport()
	r.AddHeader("field1")
	r.AddRow(NewRecord(NewDataMapBuilder().WithField("field1", 123).Build()))
	assert.Equal(t, len(r.headers), 1)
...
func TestReportShouldAddRows(t *testing.T) {
	...
	r.AddHeader("field1")
	r.AddRow(NewRecord(NewDataMapBuilder().WithField("field1", 123).Build()))
	r.AddRow(NewRecord(NewDataMapBuilder().WithField("field1", 231).Build()))
	r.AddRow(NewRecord(NewDataMapBuilder().WithField("field1", 312).Build()))
	assert.Equal(t, len(r.headers), 1)
...
func TestReportShouldAddMultipleHeadersRows(t *testing.T) {
	...
	r.AddHeader("field2")
	r.AddHeader("field3")
	r.AddRow(NewRecord(NewDataMapBuilder().WithField("field1", 123).WithField("field2", 456).WithField("field3", 789).Build()))
	r.AddRow(NewRecord(NewDataMapBuilder().WithField("field1", 231).WithField("field2", 564).WithField("field3", 897).Build()))
	r.AddRow(NewRecord(NewDataMapBuilder().WithField("field1", 312).WithField("field2", 645).WithField("field3", 978).Build()))
	assert.Equal(t, len(r.headers), 3)
	assert.Equal(t, len(r.rows), 3)
...
func TestShouldNotFetchMissingRowData(t *testing.T) {
	...
	r.AddHeader("field1")
	r.AddHeader("field2")
	r.AddRow(NewRecord(NewDataMapBuilder().WithField("field3", 789).Build()))
